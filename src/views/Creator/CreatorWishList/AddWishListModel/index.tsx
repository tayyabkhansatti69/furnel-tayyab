...
import { addWishListDetailsDefaultValues, addWishListDetailsFields, addWishListValidationSchema } from './AddWishListModel.data';
...
export  const AddWishListModal=(props:any)=> {

  const {open,handleClose,setShowCard}=props;

  const wishListForm = useForm<any>({
    resolver: yupResolver(addWishListValidationSchema),
    defaultValues: addWishListDetailsDefaultValues
  })
...
<FormProvider methods={wishListForm} onSubmit={handleSubmit(onSubmit)}>
    ...
    <Box sx={{mt:8,mb:12}}>
          ...
          <Grid container spacing={2} mt={0} mb={0}>
                    {addWishListDetailsFields?.map((item: any) => (
