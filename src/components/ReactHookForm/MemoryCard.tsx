...
export const MemoryCard = ({ flower }: any) => {
  ...
  return (
    <>
      <Card sx={{ width: 320, height: 390, marginBottom: 2, position: 'relative' }}>
        {flower.type === 'image' ? (
          <CardMedia
            component="img"
            height="250"
        ...
        <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start',background:'#F6F6F6' }}>
          ...
          <Avatar alt={flower.flowerName} src={'/images/avatars/homeAvatar.png'} sx={{ width: 56, height: 56, marginBottom: 1 }} />
          <Box>
          <Typography variant="h6">{flower?.name}</Typography>
          <Typography variant="body2">{flower?.relation}</Typography>
          </Box>
          </Box>
